...
impl ProcessManagerWrapper {
    ...
    pub fn wait_for_ready(&mut self, timeout: Duration) -> Result<(), String> {
        ...
        while start.elapsed() < timeout {
            ...
            if self.log_output.iter().any(|line| {
                line.contains("Process Manager started") ||
    ...
    /// Collect logs from PROCMAN stdout/stderr
    ...
    fn collect_final_logs(&mut self, child: &mut Child) {
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            for line in reader.lines().flatten() {
                println!("[PROCMAN-ERR] {}", line);
                self.log_output.push(format!("[ERR] {}", line));
            }
        }
    }
...
pub fn create_test_config(
    ...
    let testexe_str = testexe_path.to_string_lossy();
      ...
      standard_managed:
        control:
          execution:
            ...
            wait_delay: 100ms
          graceful_timeout: {}ms
"#, config_overrides.port, process_id, testexe_str, args, graceful_timeout);
